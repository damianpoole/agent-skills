---
name: yarn-audit-react
description: Handles yarn audit findings in React web applications. Use when yarn audit reports vulnerabilities, when dependency upgrades create React build/test risk, or when you need to safely triage and fix frontend package security issues.
---

# yarn Audit for React Apps

Use this skill to triage and fix `yarn audit` issues without blindly forcing breaking dependency updates.
...
4. Do not use `yarn audit fix --force` unless the user explicitly approves the breaking changes.
5. After changes, run the project’s relevant checks: install/lockfile update, `yarn audit`, tests, typecheck, lint, and build where available.
